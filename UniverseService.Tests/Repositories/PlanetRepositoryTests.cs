...
public class PlanetRepositoryTests : IDisposable
{
    ...
    public async Task Exists_ThenUpdate_ShouldNotCauseTrackingConflict()
    {
        ...
        // Act - This simulates the PlanetService.Update method flow
        var exists = await _repository.Exists(planet.Id);
        ...
        var result = await _repository.Update(updatedPlanet);

        // Assert
        result.Name.Should().Be("Earth Updated");
        result.Mass.Should().Be(1.1);

        var savedPlanet = await _context.Planets.AsNoTracking().FirstOrDefaultAsync(p => p.Id == planet.Id);
        savedPlanet.Should().NotBeNull();
        savedPlanet!.Name.Should().Be("Earth Updated");
    }
}

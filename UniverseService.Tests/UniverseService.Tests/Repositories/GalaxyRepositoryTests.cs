...
public class GalaxyRepositoryTests : IDisposable
{
    ...
    [Fact]
    public async Task Exists_ThenUpdate_ShouldNotCauseTrackingConflict()
    {
        // Arrange
        ...
        // Act - This simulates the GalaxyService.Update method flow
        var exists = await _repository.Exists(galaxy.Id);
        exists.Should().BeTrue();

        // Create a new instance with same ID (simulates incoming request)
        var updatedGalaxy = new Galaxy
        {
            Id = galaxy.Id,
            Name = "Milky Way Updated",
            Type = "Spiral",
        ...
        // This should NOT throw InvalidOperationException about entity tracking
        var result = await _repository.Update(updatedGalaxy);

        // Assert
        result.Name.Should().Be("Milky Way Updated");
        result.StarCount.Should().Be(300000000000);

        var savedGalaxy = await _context.Galaxies.AsNoTracking().FirstOrDefaultAsync(g => g.Id == galaxy.Id);
        savedGalaxy.Should().NotBeNull();
        savedGalaxy!.Name.Should().Be("Milky Way Updated");
    }
}
